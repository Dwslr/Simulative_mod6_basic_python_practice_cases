...
# Ñäåëàë èíòåðåñóþùèå èìïîðòû è ôóíêöèþ ÷òåíèÿ csv ôàéëà ñ èñõîäíûìè äàííûìè
...
def calc_mark_cost(data: list):
    ...
    with open(
        ...
        for platf, city in platf_data.items():
            ...
            print(f"Äëÿ ãðóïïû {platf}:", file=wf)

            for name_city, cost in sorted_cities:
                cost_share = round(cost / sum(city.values()) * 100, 2)
                print(
                    f"- Ãîðîä: {name_city}, äîëÿ çàòðàò íà ðåêëàìó: {cost_share}%",
                    file=wf,
                )
    ...
    with open(csv_path, "r") as op_csv:
        ...
        for city, click, cost in opt_data:
            if city not in city_data:
                city_data[city] = {
                    "Ãîðîä": city,
                    "Êîëè÷åñòâî êëèêîâ": click,
                    "Ñóììàðíûé áþäæåò": cost,
                }
            else:
                city_data[city]["Êîëè÷åñòâî êëèêîâ"] += click
                city_data[city]["Ñóììàðíûé áþäæåò"] += cost

        res = list(city_data.values())
        # print(res)

        sorted_res = sorted(res, key=lambda x: x["Ãîðîä"])
        # print(sorted_res)
